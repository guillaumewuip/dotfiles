...
map("n", "*", 
  ...
  { desc = "Search word under cursor in file (forward)" }
)

map("n", "#",
	[[*<Cmd>lua require('hlslens').start()<CR>]],
	{ desc = "Search word under cursor in file (backward)"}
)
...
vim.api.nvim_create_autocmd('LspAttach', {
  ...
  callback = function()
    ...
    map("n", "<leader>an", function()
			vim.diagnostic.goto_nex({ float = { border = "rounded" } })
		end, { desc = "Goto next" })
